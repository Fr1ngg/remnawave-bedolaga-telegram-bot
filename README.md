...
# ===== GOOGLE DRIVE ПОДПИСКА =====
# Включить публикацию подписок в Google Drive
GDRIVE_ENABLED=false
# Путь к JSON-файлу сервисного аккаунта (если используется)
GDRIVE_SERVICE_ACCOUNT_FILE=
# JSON сервисного аккаунта в формате строки (альтернатива файлу)
GDRIVE_SERVICE_ACCOUNT_INFO=
# ID папки для размещения файлов подписки (опционально)
GDRIVE_SUBSCRIPTIONS_FOLDER_ID=
# Формат ссылки подписки, которую нужно выгружать (clash, singbox, mihomo, json, v2ray-json, singbox-legacy, stash)
GDRIVE_SUBSCRIPTION_CLIENT_TYPE=clash
# Шаблон имени файла. Доступные плейсхолдеры: {short_uuid}, {client_type}, {extension}
GDRIVE_FILE_NAME_TEMPLATE=subscription-{short_uuid}.{extension}
# Шаблон ссылки для выдачи пользователю. Доступные плейсхолдеры: {file_id}
GDRIVE_SHARE_LINK_TEMPLATE=https://drive.google.com/uc?id={file_id}&export=download
# Автоматически делать файл доступным всем по ссылке
...
- ☁️ Автообновляемые ссылки подписки через Google Drive (опционально)
