...
По умолчанию бот **не запускает универсальные миграции** при старте. Чтобы включить их автоматическое выполнение, установите переменную окружения `ENABLE_AUTO_MIGRATION=true` (например, в файле `.env` или через параметры контейнера).

Если база данных пуста (первый запуск), бот выполнит стартовую миграцию автоматически даже при `ENABLE_AUTO_MIGRATION=false`, чтобы создать необходимые таблицы. Для дальнейших обновлений схемы миграции всё равно нужно запускать вручную.

Если вы оставляете автоматическую миграцию отключенной и запускаете бот против уже существующей базы данных, обязательно выполните универсальную миграцию вручную до запуска:

```bash
docker compose run --rm remnawave_bot \
...
Или временно запустите контейнер/процесс с переменной `ENABLE_AUTO_MIGRATION=true`, чтобы миграция выполнилась один раз.
