...
### 🔌 Admin API для веб-админки

| Переменная | Описание | Пример |
|------------|----------|--------|
| `ADMIN_API_ENABLED` | Включает HTTP API для интеграции с внешней админ-панелью. | `true` |
| `ADMIN_API_TOKEN` | Секретный токен доступа (используется в заголовке `Authorization: Bearer`). | `super-secret-token` |
| `ADMIN_API_HOST` | Хост для запуска API. | `0.0.0.0` |
| `ADMIN_API_PORT` | Порт для подключения к API. | `8085` |
| `ADMIN_API_ALLOWED_IPS` | Список разрешенных IP через запятую (оставьте пустым, чтобы разрешить все). | `10.0.0.2,10.0.0.3` |
| `ADMIN_API_CORS_ORIGINS` | Разрешенные Origins для браузера (через запятую). Пустое значение — `*`. | `https://admin.example.com` |
