...
class ExternalAdminApiKeyOwnedByAnotherAdminError(RuntimeError):
    """Raised when API key for the target Telegram ID exists under another admin."""
...
class ExternalAdminApiKeysService:
    ...
    async def ensure_key(
        ...
        *,
        ...
        self._ensure_token()
        existing_for_target = await get_api_key_by_target(
            db,
            target_telegram_id=target_telegram_id,
        )
        if (
            existing_for_target
            and existing_for_target.creator_user_id != creator_user_id
        ):
            raise ExternalAdminApiKeyOwnedByAnotherAdminError(
                "API key for this Telegram ID is owned by another administrator",
            )
...
__all__ = [
    ...
    "ExternalAdminApiKeyOwnedByAnotherAdminError",
    "ApiKeyResult",
]
