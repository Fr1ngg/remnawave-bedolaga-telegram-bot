...
class ExternalAdminApiKeyAlreadyExistsError(RuntimeError):
    """Raised when another administrator already created a key for the target."""

    def __init__(self, target_telegram_id: int):
        super().__init__(
            f"API key for target Telegram ID {target_telegram_id} already exists",
        )
...
class ExternalAdminApiKeysService:
    ...
    async def ensure_key(
        ...
        *,
        ...
        self._ensure_token()
        existing = await get_api_key_by_target(
            db,
        ...
        if existing:
            ...
            raise ExternalAdminApiKeyAlreadyExistsError(target_telegram_id)
...
__all__ = [
    ...
    "ExternalAdminApiKeyAlreadyExistsError",
    "ApiKeyResult",
]
