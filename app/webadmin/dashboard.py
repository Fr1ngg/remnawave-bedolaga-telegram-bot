...
from app.webadmin.serializers import (
    serialize_server,
    serialize_subscription_listing,
    ...
    *,
    ...
    if status_filter:
        ...
        if normalized_status == "completed":
            filters.append(Transaction.is_completed.is_(True))
        elif normalized_status == "pending":
            filters.append(Transaction.is_completed.is_(False))

    if type_filter:
        raw_types = [value.strip() for value in type_filter.split(",") if value.strip()]
        if raw_types:
            filters.append(Transaction.type.in_(raw_types))
    ...
    if search:
        ...
        if normalized:
            ...
            search_conditions: List[Any] = [
                ...
                func.lower(Transaction.payment_method).like(pattern),
            ]

            name_conditions: List[Any] = []
            name_conditions.append(func.lower(User.username).like(pattern))
            name_conditions.append(func.lower(User.first_name).like(pattern))
            name_conditions.append(func.lower(User.last_name).like(pattern))
            search_conditions.append(Transaction.user.has(or_(*name_conditions)))
            ...
            if normalized.isdigit():
                ...
                search_conditions.extend(
                    [
                        ...
                        Transaction.user_id == numeric_value,
                        Transaction.amount_kopeks == numeric_value,
                        Transaction.amount_kopeks == numeric_value * 100,
                    ]
                )
    ...
    return [serialize_transaction(tx, include_user=True) for tx in transactions], int(total)
    ...
    *,
    ...
    if status_filter:
        ...
        if normalized_status == "active":
            ...
            filters.append(Subscription.status == SubscriptionStatus.DISABLED.value)
    ...
    if search:
        ...
        if normalized:
            ...
            if normalized.isdigit():
                numeric_value = int(normalized)
                search_conditions.extend(
                    [Subscription.id == numeric_value, Subscription.user_id == numeric_value]
                )
            filters.append(or_(*search_conditions))
    ...
    return [serialize_subscription_listing(subscription) for subscription in subscriptions], int(total)
