...
async def create_admin_api_clients_table():
    table_exists = await check_table_exists("admin_api_clients")
    if table_exists:
        logger.info("Таблица admin_api_clients уже существует")
        return True
    ...
    try:
        async with engine.begin() as conn:
            ...
            if db_type == "sqlite":
                create_sql = """
                CREATE TABLE admin_api_clients (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    ...
                    basic_username VARCHAR(255) NULL,
                    cookie_key VARCHAR(255) NULL,
                    cookie_value_hash VARCHAR(128) NULL,
                ...
                CREATE INDEX idx_admin_api_clients_active ON admin_api_clients(is_active);
                CREATE INDEX idx_admin_api_clients_auth_type ON admin_api_clients(auth_type);
                """
                ...
                logger.error(f"Неподдерживаемый тип БД для таблицы admin_api_clients: {db_type}")
                return False

            await conn.execute(text(create_sql))
            logger.info("Таблица admin_api_clients успешно создана")
            return True

    except Exception as error:
        logger.error(f"Ошибка создания таблицы admin_api_clients: {error}")
        return False


async def create_admin_api_audit_log_table():
    table_exists = await check_table_exists("admin_api_audit_log")
    if table_exists:
        logger.info("Таблица admin_api_audit_log уже существует")
        return True
        ...
        async with engine.begin() as conn:
            ...
            if db_type == "sqlite":
                ...
                CREATE TABLE admin_api_audit_log (
                    ...
                    response_time_ms REAL NULL,
                    metadata_json JSON NULL,
                    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                    FOREIGN KEY (client_id) REFERENCES admin_api_clients(id) ON DELETE SET NULL
                );

                CREATE INDEX idx_admin_api_audit_created_at ON admin_api_audit_log(created_at);
                CREATE INDEX idx_admin_api_audit_client_id ON admin_api_audit_log(client_id);
                CREATE INDEX idx_admin_api_audit_token_prefix ON admin_api_audit_log(token_prefix);
                """
                ...
                CREATE TABLE admin_api_audit_log (
                    id SERIAL PRIMARY KEY,
                    client_id INTEGER NULL REFERENCES admin_api_clients(id) ON DELETE SET NULL,
                    token_prefix VARCHAR(16) NULL,
                    auth_type VARCHAR(20) NULL,
                    method VARCHAR(10) NOT NULL,
                    path VARCHAR(255) NOT NULL,
                ...
                logger.error(f"Неподдерживаемый тип БД для таблицы admin_api_audit_log: {db_type}")
                return False

            await conn.execute(text(create_sql))
            logger.info("Таблица admin_api_audit_log успешно создана")
            return True

    except Exception as error:
        logger.error(f"Ошибка создания таблицы admin_api_audit_log: {error}")
        return False
...
async def run_universal_migration():
    ...
    try:
        ...
        logger.info("=== СОЗДАНИЕ ТАБЛИЦЫ ADMIN API CLIENTS ===")
        admin_clients_created = await create_admin_api_clients_table()
        if admin_clients_created:
            logger.info("✅ Таблица admin_api_clients готова")
        else:
            logger.warning("⚠️ Проблемы с таблицей admin_api_clients")

        logger.info("=== СОЗДАНИЕ ТАБЛИЦЫ ADMIN API AUDIT LOG ===")
        admin_audit_created = await create_admin_api_audit_log_table()
        if admin_audit_created:
            logger.info("✅ Таблица admin_api_audit_log готова")
        else:
            logger.warning("⚠️ Проблемы с таблицей admin_api_audit_log")
