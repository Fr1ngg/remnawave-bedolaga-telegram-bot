...
async def check_constraint_exists(table_name: str, constraint_name: str) -> bool:
    try:
        async with engine.begin() as conn:
            ...
            if db_type == "sqlite":
                # В SQLite встроенных ограничений нет, используем альтернативный способ
                try:
                    result = await conn.execute(text(f"PRAGMA foreign_key_list('{table_name}')"))
                    rows = result.fetchall()
                    return any(row[5] == constraint_name for row in rows if len(row) > 5)
                except:
                    # Если таблица не существует или другая ошибка - возвращаем False
                    return False
...
async def check_index_exists(table_name: str, index_name: str) -> bool:
    try:
        async with engine.begin() as conn:
            ...
            if db_type == "sqlite":
                try:
                    result = await conn.execute(text("PRAGMA index_list(:table_name)"), {"table_name": table_name})
                    rows = result.fetchall()
                    return any(row[1] == index_name for row in rows if len(row) > 1)
                except:
                    # Если таблица не существует или другая ошибка - возвращаем False
                    return False
...
async def create_cryptobot_payments_table():
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'cryptobot_payments')
        if table_exists:
            logger.info("Таблица cryptobot_payments уже существует")
            return True

        # Проверяем существование пользовательской таблицы перед созданием FK
        # В целях безопасности и совместимости, проверяем только основные таблицы
        users_table_exists = await table_exists(session, 'users')
        # Не проверяем transactions, т.к. может быть поле SET NULL и таблица может не существовать
        # Проверим только users, который является обязательным
        if not users_table_exists:
            logger.warning("Таблица users не существует, но нужна для foreign key в cryptobot_payments. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                db_type = await get_database_type()
                ...
                logger.error(f"Ошибка создания таблицы cryptobot_payments без FK: {e}")
                return False
        else:
            # Создаем таблицу с FK, если все зависимости существуют
            try:
                db_type = await get_database_type()
                ...
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать cryptobot_payments с FK, создаём без них: {e}")
                try:
                    db_type = await get_database_type()
                    ...
                    logger.error(f"Ошибка создания таблицы cryptobot_payments без FK: {e2}")
                    return False
        ...
        try:
            ...
            logger.info("Таблица cryptobot_payments создана")
            ...
            for idx_sql in indexes:
                try:
                    ...
                    logger.info(f"✅ Индекс создан: {idx_sql}")
                except Exception as e:
                    logger.warning(f"⚠️ Ошибка при создании индекса '{idx_sql}': {e}")
...
async def create_heleket_payments_table():
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'heleket_payments')
        if table_exists:
        ...
        # Проверяем существование пользовательской таблицы перед созданием FK
        users_table_exists = await table_exists(session, 'users')
        # Проверим только users, который является обязательным, остальные могут быть опциональными
        if not users_table_exists:
            logger.warning("Таблица users не существует, но нужна для foreign key в heleket_payments. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                db_type = await get_database_type()
                ...
                logger.error(f"Ошибка создания таблицы heleket_payments без FK: {e}")
                return False
        else:
            # Создаем таблицу с FK, если все зависимости существуют
            try:
                ...
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать heleket_payments с FK, создаём без них: {e}")
                try:
                    db_type = await get_database_type()
                    ...
                    logger.error(f"Ошибка создания таблицы heleket_payments без FK: {e2}")
                    return False

        logger.info("Таблица heleket_payments создана")
...
async def create_mulenpay_payments_table():
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'mulenpay_payments')
        if table_exists:
        ...
        # Проверим только users, который является обязательным, остальные могут быть опциональными
        if not users_table_exists:
            logger.warning("Таблица users не существует, но нужна для foreign key в mulenpay_payments. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                db_type = await get_database_type()
                ...
                logger.error(f"Ошибка создания таблицы mulenpay_payments без FK: {e}")
                return False
        else:
            # Создаем таблицу с FK, если все зависимости существуют
            try:
                ...
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать mulenpay_payments с FK, создаём без них: {e}")
                try:
                    db_type = await get_database_type()
                    ...
                    logger.error(f"Ошибка создания таблицы mulenpay_payments без FK: {e2}")
                    return False

        logger.info("Таблица mulenpay_payments создана")
...
async def ensure_mulenpay_payment_schema() -> bool:
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            column_exists = await column_exists(session, "mulenpay_payments", "mulen_payment_id")
            paid_at_column_exists = await column_exists(session, "mulenpay_payments", "paid_at")
            ...
            if not paid_at_column_exists:
                ...
                await session.execute(text(alter_paid_at_sql))
            ...
            if not index_exists:
                ...
                try:
                    ...
                    logger.warning(f"⚠️ Ошибка при создании индекса idx_mulenpay_payment_id: {e}")
            else:
...
async def create_pal24_payments_table():
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'pal24_payments')
        if table_exists:
        ...
        # Проверим только users, который является обязательным, остальные могут быть опциональными
        if not users_table_exists:
            logger.warning("Таблица users не существует, но нужна для foreign key в pal24_payments. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                db_type = await get_database_type()
                ...
                logger.error(f"Ошибка создания таблицы pal24_payments без FK: {e}")
                return False
        else:
            # Создаем таблицу с FK, если все зависимости существуют
            try:
                ...
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать pal24_payments с FK, создаём без них: {e}")
                try:
                    db_type = await get_database_type()
                    ...
                    logger.error(f"Ошибка создания таблицы pal24_payments без FK: {e2}")
                    return False

        logger.info("Таблица pal24_payments создана")
...
async def create_wata_payments_table():
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'wata_payments')
        if table_exists:
        ...
        # Проверим только users, который является обязательным, остальные могут быть опциональными
        if not users_table_exists:
            logger.warning("Таблица users не существует, но нужна для foreign key в wata_payments. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                db_type = await get_database_type()
                ...
                logger.error(f"Ошибка создания таблицы wata_payments без FK: {e}")
                return False
        else:
            # Создаем таблицу с FK, если все зависимости существуют
            try:
                ...
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать wata_payments с FK, создаём без них: {e}")
                try:
                    db_type = await get_database_type()
                    ...
                    logger.error(f"Ошибка создания таблицы wata_payments без FK: {e2}")
                    return False

        logger.info("Таблица wata_payments создана")
...
async def ensure_wata_payment_schema() -> bool:
    try:
        async with AsyncSessionLocal() as session:
            ...
            payment_link_column_exists = await column_exists(
                session, "wata_payments", "payment_link_id"
            )
            order_id_column_exists = await column_exists(session, "wata_payments", "order_id")
            ...
            if payment_link_column_exists:
                # Для функции enforce_wata_payment_link_constraints нужна отдельная транзакция
                async with engine.begin() as conn:
                ...
                if not order_id_column_exists:
                    ...
                    if db_type in {"sqlite", "postgresql"}:
                        try:
                            ...
                            logger.warning(f"⚠️ Ошибка при создании индекса idx_wata_order_id: {e}")
                    elif db_type == "mysql":
                        try:
                            await session.execute(
...
async def create_discount_offers_table():
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'discount_offers')
        if table_exists:
            logger.info("Таблица discount_offers уже существует")
            return True

        # Проверяем существование базовых таблиц перед созданием FK
        users_table_exists = await table_exists(session, 'users')
        
        if not users_table_exists:
            logger.warning("Таблица users не существует, но нужна для foreign key в discount_offers. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                db_type = await get_database_type()
                ...
                logger.error(f"Ошибка создания таблицы discount_offers без FK: {e}")
                return False
        else:
            # Проверяем только наличие subscriptions, т.к. для него используется SET NULL
            subscriptions_table_exists = await table_exists(session, 'subscriptions')
            ...
            try:
                ...
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать discount_offers с FK, создаём без них: {e}")
                try:
                    db_type = await get_database_type()
                    ...
                    logger.error(f"Ошибка создания таблицы discount_offers без FK: {e2}")
                    return False

        logger.info("Таблица discount_offers создана")
        ...
        try:
            ...
            """))
            logger.info("✅ Индекс ix_discount_offers_user_type создан")
        except Exception as e:
...
async def ensure_discount_offer_columns():
    try:
        async with AsyncSessionLocal() as session:
            effect_exists = await column_exists(session, 'discount_offers', 'effect_type')
            extra_exists = await column_exists(session, 'discount_offers', 'extra_data')
...
async def ensure_user_promo_offer_discount_columns():
    try:
        async with AsyncSessionLocal() as session:
            percent_exists = await column_exists(session, 'users', 'promo_offer_discount_percent')
            source_exists = await column_exists(session, 'users', 'promo_offer_discount_source')
            expires_exists = await column_exists(session, 'users', 'promo_offer_discount_expires_at')
...
async def ensure_promo_offer_template_active_duration_column() -> bool:
    try:
        async with AsyncSessionLocal() as session:
            column_exists = await column_exists(session, 'promo_offer_templates', 'active_discount_hours')
...
async def create_promo_offer_templates_table():
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'promo_offer_templates')
        if table_exists:
        ...
        users_table_exists = await table_exists(session, 'users')
        if not users_table_exists:
            logger.error("Таблица users не существует, но нужна для foreign key в promo_offer_templates")
            return False
        ...
        try:
            ...
            logger.info("Таблица promo_offer_templates создана")
            ...
            try:
                if db_type in ['postgresql', 'sqlite']:
                    ...
                    """))
                logger.info("✅ Индекс ix_promo_offer_templates_type создан")
            except Exception as e:
                logger.warning(f"⚠️ Ошибка при создании индекса: {e}")
...
async def create_main_menu_buttons_table() -> bool:
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'main_menu_buttons')
        if table_exists:
        ...
        try:
            ...
            logger.info("Таблица main_menu_buttons создана")
            ...
            try:
                if db_type in ['postgresql', 'sqlite']:
                    ...
                    """))
                logger.info("✅ Индекс ix_main_menu_buttons_order создан")
            except Exception as e:
                logger.warning(f"⚠️ Ошибка при создании индекса: {e}")
...
async def create_promo_offer_logs_table() -> bool:
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'promo_offer_logs')
        if table_exists:
        ...
        # Проверяем существование базовых таблиц перед созданием FK
        users_table_exists = await table_exists(session, 'users')
        discount_offers_table_exists = await table_exists(session, 'discount_offers')
        
        # Создаём таблицу в зависимости от наличия зависимостей
        db_type = await get_database_type()
        
        if not users_table_exists or not discount_offers_table_exists:
            logger.warning("Таблицы users или discount_offers не существуют, но нужны для FK в promo_offer_logs. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                if db_type == 'sqlite':
                    await session.execute(text("""
                ...
                logger.error(f"Ошибка создания таблицы promo_offer_logs без FK: {e}")
                return False
        else:
            # Обе зависимости существуют, создаём с FK
            try:
                if db_type == 'sqlite':
                    await session.execute(text("""
                        CREATE TABLE promo_offer_logs (
                ...
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать promo_offer_logs с FK, создаём без них: {e}")
                try:
                    if db_type == 'sqlite':
                        await session.execute(text("""
                            CREATE TABLE promo_offer_logs (
                    ...
                    logger.error(f"Ошибка создания таблицы promo_offer_logs без FK: {e2}")
                    return False

        logger.info("Таблица promo_offer_logs создана")
...
async def create_subscription_temporary_access_table():
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'subscription_temporary_access')
        if table_exists:
        ...
        # Проверим, что хотя бы одна из основных таблиц существует
        if not subscriptions_table_exists or not discount_offers_table_exists:
            logger.warning("Таблицы subscriptions или discount_offers не существуют, но нужны для FK в subscription_temporary_access. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                db_type = await get_database_type()
                ...
                logger.error(f"Ошибка создания таблицы subscription_temporary_access без FK: {e}")
                return False
        else:
            # Обе таблицы существуют, создаём с FK
            try:
                db_type = await get_database_type()
                ...
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать subscription_temporary_access с FK, создаём без них: {e}")
                try:
                    db_type = await get_database_type()
                    ...
                    logger.error(f"Ошибка создания таблицы subscription_temporary_access без FK: {e2}")
                    return False

        logger.info("Таблица subscription_temporary_access создана")
...
async def create_user_messages_table():
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'user_messages')
        if table_exists:
        ...
        # Проверяем существование пользовательской таблицы перед созданием FK
        users_table_exists = await table_exists(session, 'users')
        # Проверим, нужно ли создавать с FK или без
        if not users_table_exists:
            logger.warning("Таблица users не существует, но нужна для foreign key в user_messages. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                db_type = await get_database_type()
                ...
                logger.error(f"Ошибка создания таблицы user_messages без FK: {e}")
                return False
        else:
            # Создаем таблицу с FK, если зависимость существует
            try:
                db_type = await get_database_type()
                ...
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать user_messages с FK, создаём без них: {e}")
                try:
                    db_type = await get_database_type()
                    ...
                    logger.error(f"Ошибка создания таблицы user_messages без FK: {e2}")
                    return False

        logger.info("Таблица user_messages создана")
...
async def ensure_promo_groups_setup():
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            promo_table_exists = await table_exists(session, "promo_groups")
            ...
            if not promo_table_exists:
                ...
                if db_type == "sqlite":
                    ...
                    try:
                        await session.execute(
                            text(
                                "CREATE UNIQUE INDEX uq_promo_groups_name ON promo_groups(name)"
                            )
                        )
                    except Exception as e:
                        logger.warning(f"⚠️ Ошибка при создании индекса: {e}")
                    ...
                    try:
                        ...
                        logger.warning(f"⚠️ Ошибка при добавлении ограничения: {e}")
                elif db_type == "mysql":
                    await session.execute(
                        text(
                            """
            ...
            # Работаем с отдельной сессией для добавления столбцов
            column_exists = await column_exists(session, "promo_groups", "period_discounts")
            ...
            auto_assign_column_exists = await column_exists(
                session, "promo_groups", "auto_assign_total_spent_kopeks"
            )
            ...
            addon_discount_column_exists = await column_exists(
                session, "promo_groups", "apply_discounts_to_addons"
            )
            priority_column_exists = await column_exists(
                session, "promo_groups", "priority"
            )
            ...
            column_exists = await column_exists(session, "users", "promo_group_id")

            if not column_exists:
                if db_type == "sqlite":
                    await session.execute(text("ALTER TABLE users ADD COLUMN promo_group_id INTEGER"))
                elif db_type == "postgresql":
                    await session.execute(text("ALTER TABLE users ADD COLUMN promo_group_id INTEGER"))
                elif db_type == "mysql":
                    await session.execute(text("ALTER TABLE users ADD COLUMN promo_group_id INT"))
                else:
            ...
            auto_promo_flag_exists = await column_exists(
                session, "users", "auto_promo_group_assigned"
            )
            ...
            threshold_column_exists = await column_exists(
                session, "users", "auto_promo_group_threshold_kopeks"
            )
            ...
            # Завершаем транзакцию для текущих изменений
            await session.commit()

            # Работаем с готовой таблицей
            ...
            if row:
                ...
                if not row[1]:
                    await session.execute(
                        ...
                        {"is_default": True if db_type != "sqlite" else 1, "group_id": default_group_id},
                    )
            else:
                result = await session.execute(
                    text(
                    ...
                    {"is_default": True if db_type != "sqlite" else 1},
                )
                ...
                if existing_default:
                    ...
                    if addon_discount_column_exists and priority_column_exists:
                        ...
                        """
                        insert_params["apply_discounts_to_addons"] = True if db_type != "sqlite" else 1
            ...
            if db_type == "postgresql":
                ...
                try:
                    ...
                    logger.info("Установлен NOT NULL для users.promo_group_id")
                except Exception as e:
                    logger.warning(
...
async def add_welcome_text_is_enabled_column():
    async with AsyncSessionLocal() as session:
        column_exists = await column_exists(session, 'welcome_texts', 'is_enabled')
        if column_exists:
...
async def create_welcome_texts_table():
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'welcome_texts')
        if table_exists:
        ...
        # Проверим, нужно ли создавать с FK или без
        if not users_table_exists:
            logger.warning("Таблица users не существует, но нужна для foreign key в welcome_texts. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                db_type = await get_database_type()
                ...
                logger.error(f"Ошибка создания таблицы welcome_texts без FK: {e}")
                return False
        else:
            # Создаем таблицу с FK, если зависимость существует
            try:
                ...
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать welcome_texts с FK, создаём без них: {e}")
                try:
                    db_type = await get_database_type()
                    ...
                    logger.error(f"Ошибка создания таблицы welcome_texts без FK: {e2}")
                    return False

        logger.info("Таблица welcome_texts создана")
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            for field_name, field_type in media_fields.items():
                field_exists = await column_exists(session, 'broadcast_history', field_name)
...
async def add_ticket_reply_block_columns():
    try:
        async with AsyncSessionLocal() as session:
            col_perm_exists = await column_exists(session, 'tickets', 'user_reply_block_permanent')
            col_until_exists = await column_exists(session, 'tickets', 'user_reply_block_until')
...
async def add_ticket_sla_columns():
    try:
        async with AsyncSessionLocal() as session:
            col_exists = await column_exists(session, 'tickets', 'last_sla_reminder_at')
            if col_exists:
                return True
            db_type = await get_database_type()
            if db_type == 'sqlite':
...
async def add_subscription_crypto_link_column() -> bool:
    async with AsyncSessionLocal() as session:
        column_exists = await column_exists(session, 'subscriptions', 'subscription_crypto_link')
        if column_exists:
        ...
        try:
            ...
            if db_type == 'sqlite':
                await session.execute(text("ALTER TABLE subscriptions ADD COLUMN subscription_crypto_link TEXT"))
            elif db_type == 'postgresql':
                await session.execute(text("ALTER TABLE subscriptions ADD COLUMN subscription_crypto_link VARCHAR"))
            elif db_type == 'mysql':
                await session.execute(text("ALTER TABLE subscriptions ADD COLUMN subscription_crypto_link VARCHAR(512)"))
            else:
...
async def add_referral_system_columns():
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            column_exists = await column_exists(session, 'users', 'has_made_first_topup')
            ...
            if not column_exists:
                ...
                await session.execute(text(f"ALTER TABLE users ADD COLUMN has_made_first_topup {column_def}"))
...
async def create_subscription_conversions_table():
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, 'subscription_conversions')
        if table_exists:
        ...
        # Проверим, нужно ли создавать с FK или без
        if not users_table_exists:
            logger.warning("Таблица users не существует, но нужна для foreign key в subscription_conversions. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                db_type = await get_database_type()
                ...
                logger.error(f"Ошибка создания таблицы subscription_conversions без FK: {e}")
                return False
        else:
            # Создаем таблицу с FK, если зависимость существует
            try:
                ...
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать subscription_conversions с FK, создаём без них: {e}")
                try:
                    db_type = await get_database_type()
                    ...
                    logger.error(f"Ошибка создания таблицы subscription_conversions без FK: {e2}")
                    return False

        logger.info("Таблица subscription_conversions создана")
...
async def ensure_server_promo_groups_setup() -> bool:
    ...
    try:
        async with AsyncSessionLocal() as session:
            table_exists = await table_exists(session, "server_squad_promo_groups")
            ...
            if not table_exists:
                ...
                try:
                    if db_type in ["postgresql", "sqlite"]:
                        ...
                        await session.execute(text("""
                            CREATE INDEX idx_server_squad_promo_groups_promo ON server_squad_promo_groups(promo_group_id)
                        """))
                    logger.info("✅ Индекс idx_server_squad_promo_groups_promo создан")
                except Exception as e:
                    logger.warning(f"⚠️ Ошибка при создании индекса: {e}")
            ...
            # Выполняем операции с данными в отдельной транзакции
            async with AsyncSessionLocal() as data_session:
                default_query = (
                    "SELECT id FROM promo_groups WHERE is_default IS TRUE LIMIT 1"
                    if db_type == "postgresql"
                    else "SELECT id FROM promo_groups WHERE is_default = 1 LIMIT 1"
                )
                default_result = await data_session.execute(text(default_query))
                ...
                servers_result = await data_session.execute(text("SELECT id FROM server_squads"))
                server_ids = [row[0] for row in servers_result.fetchall()]

                assigned_count = 0
                for server_id in server_ids:
                    existing = await data_session.execute(
                        text(
...
async def add_server_trial_flag_column() -> bool:
    async with AsyncSessionLocal() as session:
        column_exists = await column_exists(session, 'server_squads', 'is_trial_eligible')
        if column_exists:
...
async def create_system_settings_table() -> bool:
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, "system_settings")
        if table_exists:
...
async def create_web_api_tokens_table() -> bool:
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, "web_api_tokens")
        if table_exists:
...
async def create_privacy_policies_table() -> bool:
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, "privacy_policies")
        if table_exists:
...
async def create_public_offers_table() -> bool:
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, "public_offers")
        if table_exists:
...
async def create_faq_settings_table() -> bool:
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, "faq_settings")
        if table_exists:
...
async def create_faq_pages_table() -> bool:
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, "faq_pages")
        if table_exists:
...
async def add_promo_group_priority_column() -> bool:
    ...
    async with AsyncSessionLocal() as session:
        column_exists = await column_exists(session, 'promo_groups', 'priority')
        if column_exists:
        ...
        try:
            ...
            try:
                ...
                logger.info("✅ Индекс для сортировки по приоритету создан")
            except Exception as e:
                logger.warning(f"⚠️ Ошибка при создании индекса: {e}")
...
async def create_user_promo_groups_table() -> bool:
    ...
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, "user_promo_groups")
        if table_exists:
        ...
        # Проверяем существование базовых таблиц перед созданием FK
        users_table_exists = await table_exists(session, 'users')
        promo_groups_table_exists = await table_exists(session, 'promo_groups')
        
        # Создаём таблицу в зависимости от наличия зависимостей
        if not users_table_exists or not promo_groups_table_exists:
            logger.warning("Таблицы users или promo_groups не существуют, но нужны для FK в user_promo_groups. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                db_type = await get_database_type()
                ...
                logger.info("Таблица user_promo_groups создана без FK")
            except Exception as e:
                logger.error(f"❌ Ошибка создания таблицы user_promo_groups без FK: {e}")
                return False
        else:
            # Обе зависимости существуют, создаём с FK
            try:
                ...
                logger.info("Таблица user_promo_groups создана с FK")
            except Exception as e:
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать user_promo_groups с FK, создаём без них: {e}")
                try:
                    db_type = await get_database_type()
                    ...
                    logger.info("Таблица user_promo_groups создана без FK после ошибки с FK")
                except Exception as e2:
                    logger.error(f"❌ Ошибка создания таблицы user_promo_groups без FK: {e2}")
                    return False

        # Создаем индекс отдельно
        try:
            await session.execute(text("CREATE INDEX idx_user_promo_groups_user_id ON user_promo_groups(user_id)"))
            logger.info("✅ Индекс idx_user_promo_groups_user_id создан")
        except Exception as e:
            logger.warning(f"⚠️ Ошибка при создании индекса: {e}")
...
async def migrate_existing_user_promo_groups_data() -> bool:
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            result = await session.execute(text("SELECT COUNT(*) FROM user_promo_groups"))
            count = result.scalar()
            ...
            result = await session.execute(text(migrate_sql))
...
async def add_promocode_promo_group_column() -> bool:
    ...
    async with AsyncSessionLocal() as session:
        column_exists = await column_exists(session, 'promocodes', 'promo_group_id')
        if column_exists:
        ...
        try:
            ...
            if db_type == 'sqlite':
                ...
                try:
                    ...
                    logger.warning(f"⚠️ Ошибка при добавлении внешнего ключа: {e}")
                # Add index
                try:
                    await session.execute(
...
async def run_universal_migration():
    ...
    try:
        ...
        # 2. Миграции колонок в существующих таблицах
...
async def create_base_tables():
    ...
    logger.info("=== СОЗДАНИЕ БАЗОВЫХ ТАБЛИЦ ===")
    
    # Таблицы, которые не зависят от других таблиц
    base_tables = [
        create_system_settings_table,
        create_web_api_tokens_table,
        create_privacy_policies_table,
        create_public_offers_table,
        create_faq_settings_table,
        create_faq_pages_table,
        create_user_messages_table,
        create_welcome_texts_table,
        create_main_menu_buttons_table,
    ]
    
    for table_func in base_tables:
        try:
            result = await table_func()
            logger.info(f"✅ Таблица от {table_func.__name__}: {'успешно' if result else 'ошибка'}")
        except Exception as e:
            logger.error(f"❌ Ошибка при создании таблицы {table_func.__name__}: {e}")


async def migrate_columns():
    """Миграция колонок в существующих таблицах"""
    logger.info("=== МИГРАЦИЯ КОЛОНК В СУЩЕСТВУЮЩИХ ТАБЛИЦАХ ===")
    
    column_migrations = [
        add_referral_system_columns,
        add_welcome_text_is_enabled_column,
        add_media_fields_to_broadcast_history,
        add_ticket_reply_block_columns,
        add_ticket_sla_columns,
        add_subscription_crypto_link_column,
        add_server_trial_flag_column,
        add_promo_group_priority_column,
        ensure_promo_offer_template_active_duration_column,
        ensure_user_promo_offer_discount_columns,
        ensure_discount_offer_columns,
        add_promocode_promo_group_column,
    ]
    
    for mig_func in column_migrations:
        try:
            result = await mig_func()
            logger.info(f"✅ Колонки от {mig_func.__name__}: {'успешно' if result else 'ошибка'}")
        except Exception as e:
            logger.error(f"❌ Ошибка при миграции колонок {mig_func.__name__}: {e}")


async def create_new_tables():
    """Создание новых таблиц с внешними ключами"""
    logger.info("=== СОЗДАНИЕ НОВЫХ ТАБЛИЦ С FK ===")
    
    # Таблицы с внешними ключами на пользователей и другие сущности
    new_tables = [
        create_promo_groups_table,
        create_discount_offers_table,
        create_promo_offer_templates_table,
        create_promo_offer_logs_table,
        create_user_promo_groups_table,
        create_subscription_temporary_access_table,
        create_subscription_conversions_table,
        create_cryptobot_payments_table,
        create_heleket_payments_table,
        create_mulenpay_payments_table,
        create_pal24_payments_table,
        create_wata_payments_table,
    ]
    
    for table_func in new_tables:
        try:
            result = await table_func()
            logger.info(f"✅ Таблица от {table_func.__name__}: {'успешно' if result else 'ошибка'}")
        except Exception as e:
...
async def create_indexes():
    ...
    # Индексы создаются автоматически при создании таблиц, просто подтверждаем завершение
    logger.info("✅ Индексы созданы в процессе создания таблиц")
...
async def migrate_data():
    ...
    data_migrations = [
        migrate_existing_user_promo_groups_data,
        ensure_default_web_api_token,
        ensure_promo_groups_setup,
        ensure_server_promo_groups_setup,
        ensure_wata_payment_schema,
        ensure_mulenpay_payment_schema,
        migrate_discount_offer_effect_types,
        reset_discount_offer_bonuses,
        fix_foreign_keys_for_user_deletion,
        fix_subscription_duplicates_universal,
    ]
    
    for mig_func in data_migrations:
        try:
            result = await mig_func()
            logger.info(f"✅ Данные от {mig_func.__name__}: {'успешно' if result else 'ошибка'}")
        except Exception as e:
            logger.error(f"❌ Ошибка при миграции данных {mig_func.__name__}: {e}")


async def create_promo_groups_table():
    """Создание таблицы промо-групп"""
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, "promo_groups")
        if table_exists:
            logger.info("ℹ️ Таблица promo_groups уже существует")
            return True
        ...
        try:
            ...
            logger.info("✅ Таблица promo_groups создана")

            # Создаем индекс для уникального имени
            try:
                if db_type == "postgresql":
                    await session.execute(text("CREATE UNIQUE INDEX uq_promo_groups_name ON promo_groups(name)"))
                elif db_type == "sqlite":
                    await session.execute(text("CREATE UNIQUE INDEX uq_promo_groups_name ON promo_groups(name)"))
                else:  # MySQL
                    await session.execute(text("ALTER TABLE promo_groups ADD CONSTRAINT uq_promo_groups_name UNIQUE (name)"))
                logger.info("✅ Уникальный индекс/ограничение для promo_groups.name создан")
            except Exception as e:
                logger.warning(f"⚠️ Ошибка при создании уникального индекса/ограничения: {e}")
            ...
            await session.rollback()
            logger.error(f"❌ Ошибка создания таблицы promo_groups: {e}")
            return False


async def create_support_audit_logs_table():
    """Создание таблицы аудита поддержки"""
    async with AsyncSessionLocal() as session:
        table_exists = await table_exists(session, "support_audit_logs")
        if table_exists:
        ...
        # Проверяем существование базовых таблиц перед созданием FK
        users_table_exists = await table_exists(session, 'users')
        tickets_table_exists = await table_exists(session, 'tickets')
        
        # Создаём таблицу в зависимости от наличия зависимостей
        if not users_table_exists or not tickets_table_exists:
            logger.warning("Таблицы users или tickets не существуют, но нужны для FK в support_audit_logs. Создаём без FK.")
            # Создаем таблицу без внешних ключей для совместимости с разными состояниями БД
            try:
                db_type = await get_database_type()
                ...
                logger.info("Таблица support_audit_logs создана без FK")
            except Exception as e:
                logger.error(f"❌ Ошибка создания таблицы support_audit_logs без FK: {e}")
                return False
        else:
            # Обе зависимости существуют, создаём с FK
            try:
                ...
                logger.info("Таблица support_audit_logs создана с FK")
            except Exception as e:
                # Если не удалось создать с FK, попробуем создать без них
                logger.warning(f"Не удалось создать support_audit_logs с FK, создаём без них: {e}")
                try:
                    db_type = await get_database_type()
                    ...
                    logger.info("Таблица support_audit_logs создана без FK после ошибки с FK")
                except Exception as e2:
                    logger.error(f"❌ Ошибка создания таблицы support_audit_logs без FK: {e2}")
                    return False
        ...
        await session.commit()
        logger.info("✅ Таблица support_audit_logs успешно создана")
        return True
...
async def check_migration_status():
    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            status["has_made_first_topup_column"] = await column_exists(session, 'users', 'has_made_first_topup')

            status["cryptobot_table"] = await table_exists(session, 'cryptobot_payments')
            status["heleket_table"] = await table_exists(session, 'heleket_payments')
            status["user_messages_table"] = await table_exists(session, 'user_messages')
            status["welcome_texts_table"] = await table_exists(session, 'welcome_texts')
            status["privacy_policies_table"] = await table_exists(session, 'privacy_policies')
            status["public_offers_table"] = await table_exists(session, 'public_offers')
            status["subscription_conversions_table"] = await table_exists(session, 'subscription_conversions')
            status["promo_groups_table"] = await table_exists(session, 'promo_groups')
            status["server_promo_groups_table"] = await table_exists(session, 'server_squad_promo_groups')
            status["server_squads_trial_column"] = await column_exists(session, 'server_squads', 'is_trial_eligible')

            status["discount_offers_table"] = await table_exists(session, 'discount_offers')
            status["discount_offers_effect_column"] = await column_exists(session, 'discount_offers', 'effect_type')
            status["discount_offers_extra_column"] = await column_exists(session, 'discount_offers', 'extra_data')
            status["promo_offer_templates_table"] = await table_exists(session, 'promo_offer_templates')
            status["promo_offer_templates_active_discount_column"] = await column_exists(session, 'promo_offer_templates', 'active_discount_hours')
            status["promo_offer_logs_table"] = await table_exists(session, 'promo_offer_logs')
            status["subscription_temporary_access_table"] = await table_exists(session, 'subscription_temporary_access')

            status["welcome_texts_is_enabled_column"] = await column_exists(session, 'welcome_texts', 'is_enabled')
            status["users_promo_group_column"] = await column_exists(session, 'users', 'promo_group_id')
            status["promo_groups_period_discounts_column"] = await column_exists(session, 'promo_groups', 'period_discounts')
            status["promo_groups_auto_assign_column"] = await column_exists(session, 'promo_groups', 'auto_assign_total_spent_kopeks')
            status["promo_groups_addon_discount_column"] = await column_exists(session, 'promo_groups', 'apply_discounts_to_addons')
            status["users_auto_promo_group_assigned_column"] = await column_exists(session, 'users', 'auto_promo_group_assigned')
            status["users_auto_promo_group_threshold_column"] = await column_exists(session, 'users', 'auto_promo_group_threshold_kopeks')
            status["users_promo_offer_discount_percent_column"] = await column_exists(session, 'users', 'promo_offer_discount_percent')
            status["users_promo_offer_discount_source_column"] = await column_exists(session, 'users', 'promo_offer_discount_source')
            status["users_promo_offer_discount_expires_column"] = await column_exists(session, 'users', 'promo_offer_discount_expires_at')
            status["subscription_crypto_link_column"] = await column_exists(session, 'subscriptions', 'subscription_crypto_link')

            media_fields_exist = (
                await column_exists(session, 'broadcast_history', 'has_media') and
                await column_exists(session, 'broadcast_history', 'media_type') and
                await column_exists(session, 'broadcast_history', 'media_file_id') and
                await column_exists(session, 'broadcast_history', 'media_caption')
            )
