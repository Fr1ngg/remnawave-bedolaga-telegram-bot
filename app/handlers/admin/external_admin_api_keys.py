...
async def process_api_key_input(
    ...
    try:
        ...
        await state.clear()
        return
    except ExternalAdminApiKeyConflictError:
        await message.answer(
            texts.t(
                "ADMIN_API_KEYS_CONFLICT",
                (
                    "⚠️ Для <code>{telegram_id}</code> уже создан ключ другим администратором."
                    " Попросите его поделиться ключом или удалить запись."
                ),
