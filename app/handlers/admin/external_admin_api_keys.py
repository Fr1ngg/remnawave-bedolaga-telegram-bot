...
async def process_api_key_input(
    ...
    try:
        ...
        await db.commit()
    except ExternalAdminApiKeyAlreadyExistsError:
        await db.rollback()
        await message.answer(
            texts.t(
                "ADMIN_API_KEYS_EXISTS_OTHER",
                (
                    "⚠️ Ключ для <code>{telegram_id}</code> уже создан другим администратором.\n"
                    "Попросите его поделиться ключом или удалить его перед созданием нового."
                ),
            ).format(telegram_id=target_id)
        )
        return
    except ExternalAdminTokenMissingError:
        await message.answer(
