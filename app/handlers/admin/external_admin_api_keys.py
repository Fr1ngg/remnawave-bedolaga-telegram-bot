...
async def process_api_key_input(
    ...
    try:
        ...
        await state.clear()
        return
    except ExternalAdminApiKeyOwnedByAnotherAdminError:
        await message.answer(
            texts.t(
                "ADMIN_API_KEYS_TARGET_TAKEN",
                (
                    "❌ Ключ для <code>{telegram_id}</code> уже создан другим администратором.\n"
                    "Обратитесь к нему, чтобы получить значение."
                ),
